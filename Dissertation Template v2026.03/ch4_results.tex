...
After the reliable analysis, provisional mean score was calculated for each construct, and Pearson correlation was calculated among them (Section~\ref{sec:Data Analysis}), which is only a preliminary, descriptive step and cannot be treated as the confirmatory test of the hypotheses (see Section~\ref{sec:Structural Model and Hypothesis Testing}). The correlation matrix is given in Table~\ref{tab:prelim_corr} with all of the estimates being statistically significant $p < .001$.
...
The estimation of structural path coefficients was performed using bootstrapping (2000 resamples, percentile method, and two-tailed test). The results are presented in Table~\ref{tab:structural_paths}, which indicates the estimated unstandardised path coefficient ($\beta$), bootstrap standard error, $t$-statistic approximate $p$-value, and 95\% confidence interval for each path in the baseline (overall ROI) structural model.
