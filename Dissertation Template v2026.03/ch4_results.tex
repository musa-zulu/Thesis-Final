...
This chapter presents the results of the data analysis described in Chapter Three. The chapter follows the two-stage analytical strategy outlined in Section~\ref{sec:Data Analysis}. The analysis starts with reporting the sample profile, data screening, descriptive statistics, reliability, common method bias, and preliminary Pearson correlations. It then proceeds with reporting the structural equation model (PLS-SEM) analysis, which includes measurement model, discriminant validity, ROI specification, structural model and hypothesis testing, effect sizes, mediation test, and control variable effects. The chapter ends with a summary table that links all the hypotheses with their outcomes.

In this chapter, two reporting rules have been consistently applied: all Likert items were given values from 1 (Strongly Agree) to 5 (Strongly Disagree), which means that a \emph{lower} mean score indicates \emph{stronger} agreement; this is the coding rule followed in this analysis and it is the reverse of the usual rule, which assumes that higher scores indicate greater agreement. Every mean reported below is interpreted with this direction in mind. Second, the PLS-SEM statistics reported in Sections~\ref{sec:Measurement Model}--\ref{sec:Robustness and Sensitivity Analysis} were computed using a Python-based implementation of the partial least squares algorithm rather than the SmartPLS software package; both implement the same underlying algorithm, and every statistic reported in this chapter, including the Dijkstra-Henseler consistency reliability coefficient (rho\textsubscript{A}), computed directly from the fitted model's outer weights and indicator correlation matrix using Dijkstra and Henseler's (2015) closed-form estimator \autocite{Dijkstra2015}, is reported alongside Cronbach's alpha and composite reliability (Dillon-Goldstein's rho) rather than in place of them.
...
The study received a complete of 357 responses. Two answers came from individuals saying that their team is not using AI coding assistance, disqualifying them as per the rules of qualifying participants mentioned at the beginning, leaving the number of valid responses at 355, according to the first qualifying principle in this dataset. Among these 355 valid responders, there was one person who had one missing value (a workflow-based ROI value), so the complete-case method from Chapter 3 was used to exclude him, and the total number of respondents left was $N = 354$. The apparent difference of 354 resulted from the missing data exclusion applied post primary eligibility check, thereby ensuring no additional exclusion was done on the basis of role. The table summarising the process appears in Table~\ref{tab:sample_flow}.
...
Table~\ref{tab:descriptives} displays descriptive statistics for the constructs computed by calculating the mean of the constituent items. It is worth noting that each of the constructs had a low mean (all values being below 1.35 in the scale from 1 to 5) , but showed significant positive skewness and, in many cases, high kurtosis which indicated that in most cases the respondents picked ``Strongly Agree'' or ``Agree''. This phenomenon, which is called ceiling effect, is discussed further in the assessment of the common method bias and in the separate part discussing sensitivity analysis (Section~\ref{sec:Common Method Bias}) and in a dedicated sensitivity analysis (Section~\ref{sec:Robustness and Sensitivity Analysis}), where it is shown to be more than a cosmetic feature of the data: 247 of the 354 respondents (69.8\%) selected the most positive response option on every single substantive item.
...
Unlike the control sensitivity check, this comparison shows a material change: with the uniformly positive respondents excluded, the culture-to-assimilation path weakened but remained the dominant predictor of assimilation quality, the assimilation-quality-to-ROI path weakened substantially (from .356 to .096), and organisational readiness's effect on ROI strengthened further. $R^2$ for assimilation quality fell from .481 to .174, and $R^2$ for ROI fell from .816 to .750. No path reversed direction, but the size of the assimilation-quality pathway in particular is not robust to the exclusion of respondents who gave the most positive answer to every item, and roughly two-thirds of the achieved sample fall into that category. Following the instruction in Chapter Three to report such a change openly rather than to treat the primary result as the only one that matters, this is reported as a genuine limitation of the strength (though not the direction) of the mediation finding in Section~\ref{sec:Mediation Analysis Results}, and is revisited in Chapter Five alongside the common method bias result.
